...
int main(int argc,char *argv[]){
  ...
  for (int i = 0; i < n-5; i++) {
    for (int j = 0; j < n-5; j++) {
      //用いるeleファイル名
      char *filename;
      filename = './../triangle/poly/nine_holes_' + (i+1) + '_' + (j+1) + '.1.ele';
      //境界条件タイプ
...
void FEM(int n, char *filename, int x, int y){
  ...
  tmp_file_position = './tmp_data/nine_holes_position_data.csv';
  char* tmp_file_size;
  tmp_file_size = './tmp_data/nine_holes_size_data.csv';
  

  FILE *file_position;
  file_position = fopen(tmp_file_position,"a");
  fprintf(file_position, "%lf,", (x/10)*5 + (y/10)+1);
  for(i=0; i<N; i++) {
    if (coord[i][0] == 0) {
  ...
  fprintf(file_size, "%d,", 3);
  for(i=0; i<N; i++) {
    if (coord[i][0] == 0) {
      fprintf(file_size, "%d", u_dash[i]);
    }
  }
  fprintf(file_size, "\n");
  fclose(file_size);
