...
int main(int argc,char *argv[]){
  ...
  strcat(filename, "./../triangle/poly/half_four_holes_");
...
void FEM(int n, char *filename, int x, int y){
  ...
  tmp_file_position = "./tmp_data/half_four_holes_position_data.csv";
  char* tmp_file_size;
  tmp_file_size = "./tmp_data/half_four_holes_size_data.csv";
  

  FILE *file_position;
  file_position = fopen(tmp_file_position,"a");
  fprintf(file_position, "%d,", (x/5)*5 + (y/5)+1);
  for(i=0; i<N; i++) {
    if (coord[i][0] == 0) {
